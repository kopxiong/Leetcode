# -*- coding: utf-8 -*-
# 堆排序（Heapsort）是指利用堆这种数据结构所设计的一种选择排序算法。(TopK, 优先级队列等等问题)
...
# Time complexity: O(nlogn); Space complexity: O(1), in-place

# heap is stored in array or list
class Solution:
    def heapSort(self, lists):
        # 创建最大堆 (O(n)), start from the last non-child node
    ...
    def sift_down(self, lists, start, end):
        ...
        while True:
            ...
            if child > end:
                break
            # select the largest value between left-child and right-child
            if child + 1 <= end and lists[child] < lists[child + 1]:
                child += 1
            # swap the root with the largest child if not satisfy the heap property
